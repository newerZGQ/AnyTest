package com.zgq.rim.common.view.chart;
...
public class ChartView extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.com_zgq_rim_common_view_chart_ChartView, defStyleAttr, 0);
        int count = array.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = array.getIndex(i);
            if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_xylinecolor) {
                xylinecolor = array.getColor(attr, xylinecolor);

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_xylinewidth) {
                xylinewidth = (int) array.getDimension(attr,
                        TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xylinewidth,
                                getResources().getDisplayMetrics()));

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_xytextcolor) {
                xytextcolor = array.getColor(attr, xytextcolor);

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_xytextsize) {
                xytextsize = (int) array.getDimension(attr,
                        TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_PX, xytextsize,
                                getResources().getDisplayMetrics()));

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_linecolor) {
                linecolor = array.getColor(attr, linecolor);

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_bgcolor) {
                bgcolor = array.getColor(attr, bgcolor);

            } else if (attr == R.styleable.com_zgq_rim_common_view_chart_ChartView_isScroll) {
